...
/** Studio apps and direct links to the places where the work continues.
 * Icons magnify around the pointer; reduced-motion users keep a still dock. */
...
function AppIcon({ name }: { name: string }) {
  return <Image src={`/studio/${name}.svg`} alt="" width={36} height={36} className="block h-9 w-9" />
}

function SocialMark({ children }: { children: ReactNode }) {
  return <span className="flex h-9 w-9 items-center justify-center"><svg viewBox="0 0 24 24" aria-hidden className="h-5 w-5 fill-[var(--fg)]">{children}</svg></span>
}

export default function Dock({ independent = false }: { independent?: boolean }) {
  ...
  const items: Item[] = [
    { key: 'terminal', href: '/', label: 'terminal', description: 'open the studio terminal', art: <AppIcon name="terminal" /> },
    { key: 'projects', href: '/projects', label: 'projects', description: 'open the project browser', art: <AppIcon name="projects" /> },
    { key: 'reader', href: '/reader', label: 'writing', description: 'open the reader — essays and The Working Prototype', art: <AppIcon name="writing" /> },
    { key: 'tapas', href: '/tapas', label: 'tapas', description: 'open tapas — local dictation and meeting transcripts for Mac', art: <Image src="/tapas/icon.png" alt="" width={36} height={36} className="block h-9 w-9" /> },
    { key: 'substack', href: profile.links.newsletter, label: 'Substack ↗', description: 'The Working Prototype on Substack — subscribe', external: true, art: <SocialMark><path d="M22.539 8.242H1.46V5.406h21.08v2.836zM1.46 10.812V24L12 18.11 22.54 24V10.812H1.46zM22.54 0H1.46v2.836h21.08V0z" /></SocialMark> },
    { key: 'x', href: profile.links.twitter, label: 'X ↗', description: 'Follow Adilet on X — @adgapar', external: true, art: <SocialMark><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></SocialMark> },
  ]
  ...
  if (!independent && maximized && !minimized) return null
      ...
      >
        ...
        {items.map((item, i) => (
          <Fragment key={item.key}>
            {/* the rule that says the machine ends here: apps on the left,
            ...
            <Link
              ...
              href={item.href}
              onClick={() => { if (item.key === 'terminal') setWindowState({ minimized: false }) }}
